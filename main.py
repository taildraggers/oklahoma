"""Aggregates Oklahoma-located listings from every taildraggers.com
...
ones whose location ends in ", OK".
...
Jersey/New Mexico/New York/North Carolina/North Dakota/Ohio repos'
approach, with the state filter swapped to "OK".
"""
...
PAGE_TITLE = "Taildragger Ads in Oklahoma"
STATE_ABBR = "OK"
...
_OKLAHOMA_LOCATION_RE = re.compile(r",\s*OK\s*$", re.IGNORECASE)


def _is_oklahoma(location: str) -> bool:
    return bool(_OKLAHOMA_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        oklahoma_rows = [row for row in rows if _is_oklahoma(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(oklahoma_rows)} in Oklahoma")
        all_listings.extend(oklahoma_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Oklahoma listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Oklahoma listings: {len(listings)}")
