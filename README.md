...
Daily aggregator of Oklahoma-located taildragger classified listings,
...
repos' approach, with the state filter swapped to "OK".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, OK` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Oklahoma"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/oklahoma/`
...
```html
<iframe
  src="https://taildraggers.github.io/oklahoma/"
  title="Taildragger Ads in Oklahoma"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_OKLAHOMA_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
